...
def nlp_metrics(ref_file, pred_file, root_path=None):
    ...
    from ..metrics.distinct import distinct, repRate
    avg_dist_1 = sum([distinct(preds_list[i], 1) for i in range(len(preds_list))])/len(preds_list)
    avg_dist_2 = sum([distinct(preds_list[i], 2) for i in range(len(preds_list))])/len(preds_list)
    
    rep = repRate(preds_list, refs_list)
    
    return bleu, bleu_list, nist, nist_list, nist_bleu, nist_bleu_list, s_dist, c_dist, entropy_scores, 0.0, \
           rouge_l, f1_score, avg_pred_length, avg_dist_1, avg_dist_2, rep
