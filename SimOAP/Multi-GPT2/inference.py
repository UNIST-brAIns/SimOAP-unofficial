...
def main():
    ...
    # Create a random index list with the fixed random seed
    indices = list(range(len(test_dataset)))
    random.shuffle(indices)

    # Select the first `subset_size` indices from the shuffled list
    subset_indices = indices[:subset_size]

    # Create a subset of the dataset containing only 10% of the data
    test_subset = torch.utils.data.Subset(test_dataset, [0])
    ...
    def external_metrics_func(full_references, full_predictions, epoch, metric=None):
        ...
        rouge_l, f1_score, avg_length, avg_dist_1, avg_dist_2, rep = nlp_metrics(references_file_path, predictions_file_path)

        metrics = {'meteor': meteor, 'avg_len': avg_length, 'rouge-l': rouge_l, 'bleu': bleu, 'nist': nist,
                   'nist-bleu': nist_bleu, 'f1': f1_score, 'dist-1': avg_dist_1, 'dist-2': avg_dist_2, 'rep':rep}
        for name, metric in (
