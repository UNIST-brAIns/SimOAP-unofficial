...
class Trainer:
    ...
    def collate_func(self, data):
        persona_info, h, y, distractors_batch, string_persona, string_dialog = zip(*data)
        ...
        return contexts, y_out, distractors, string_persona, string_dialog
    ...
    def _eval_test(self, metric_funcs={}, external_metrics_func=None, epoch=-1, inference=False, is_best=False,
        ...
        with torch.no_grad():
            ...
            for i, (contexts, targets, distractors, string_persona, string_dialog) in enumerate(tqdm_data):
                n_sample = 20
                metrics_saver = [0 for i in range(n_sample)]
                predictions_saver = [0 for i in range(n_sample)]
                cur_references = 0
                for s in range(n_sample):
                    '''Get the loss, ppl for each batch'''
                    metrics_saver[s], lengths, enc_contexts = self._get_eval_loss(contexts, targets, distractors, metrics, i)
                    # full sequence loss
                    pred, cur_references = self._get_eval_predictions(contexts, targets, lengths, enc_contexts,
                                metrics_saver[s], metric_funcs, external_metrics_func, i)
                    predictions_saver[s] = "".join(pred)
                #batchsize = 1
                print(predictions_saver)
                best, index = filter(string_persona[0], string_dialog[0], predictions_saver)
                metrics = metrics_saver[index]
                cur_predictions = predictions_saver[index]
