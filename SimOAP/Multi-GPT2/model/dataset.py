...
class FacebookDataset(Dataset):
    ...
    def make_dataset(data, vocab, only_final=False):
        ...
        if isinstance(vocab, OpenAIGPTTokenizer) or isinstance(vocab, GPT2Tokenizer) or isinstance(vocab, Seq2seqTokenizer):
            for chat in tqdm(data):
                string_persona = "".join(chat['persona_info'])
                string_history = "".join(chat['dialog'])
                ...
                if only_final:
                    ...
                    dataset.append((persona_info, dialog[:], [], string_persona, string_history))
                else:
                    for i, replica in enumerate(chat['dialog'], 1):
                        ...
                        if not i % 2:
                            if chat['candidates']:
                                ...
                                dataset.append((persona_info, dialog[:], candidates_ids, string_persona, string_history))
                            else:
                                dataset.append((persona_info, dialog[:], [], string_persona, string_history))
            ...
            for chat in tqdm(data):
                string_persona = sum(chat['persona_info'])
                string_history = sum(chat['dialog'])
    ...
    def __getitem__(self, idx):
        persona_info, dialog, candidates, string_persona, string_dialog = self.data[idx]
        ...
        return persona_info, h, sentences[0], sentences[1:], string_persona, string_dialog
